...
function ContactForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="h-60 flex flex-col justify-evenly">
      <TextInput id="email" type="email" name="email" placeholder="Email Address"/>
      <ValidationError prefix="Email" field="email" errors={state.errors} />
      <TextInput id="number" type="number" name="number" placeholder="Contact Number"/>
      <Textarea id="message" name="message" placeholder="Your Message Here"/>
      <ValidationError prefix="Message" field="message" errors={state.errors} />
      <Button type="submit" disabled={state.submitting} outline={true} className="">
        Submit
      </Button>
    </form>
  );
